...
const EditPlanificationModal = ({ onClose, onSave, planId }) => {
  ...
  const fetchPlanification = async () => {
    setLoadingPlan(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/planifications/${planId}`);
      const plan = response.data;
      setPlanData({
        site: plan.site?._id || '',
        ligne: plan.ligne?._id || '',
        timeSlots: plan.timeSlots?.length
          ...
          : [{ startTime: new Date(0, 0, 0, 18, 0), endTime: new Date(0, 0, 0, 6, 0), intensity: 50 }],
        frequence: plan.frequence || 'Quotidien',
        mode: plan.mode || 'Manuel',
        statut: plan.statut || 'Activé',
        repetition: plan.repetition || false,
        saison: plan.saison || '',
      });
    } catch (error) {
      toast.error('Erreur lors du chargement de la planification.', toastConfig);
      console.error('Fetch planification error:', error);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/api/planifications/${planId}`, payload);
      toast.success('Planification modifiée avec succès !', toastConfig);
      onSave(response.data);
      handleCancel();
    } catch (error) {
      const errorMessage = error.response?.data?.message || 'Erreur lors de la mise à jour de la planification.';
    ...
    <div className="plan-modal-overlay">
      <div className="plan-modal-container">
        ...
        <div className="plan-modal-body">
          {loadingPlan ? (
            <div className="plan-loading">Chargement de la planification...</div>
          ) : (
...
export default EditPlanificationModal;